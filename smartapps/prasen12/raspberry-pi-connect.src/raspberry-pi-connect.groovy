...
def deviceDiscoveryPage()
{
    ...
    return dynamicPage(name:"deviceDiscoveryPage", title:"Discovery Started!", nextPage: "", refreshInterval: refreshInterval, install:false, uninstall: false) {
...
def mainPage()   {
	log.debug "mainPage() page, selected device = ${selecteddevice}"
	...
	dynamicPage(name:"mainPage", title:"Select services",  refreshInterval: 10, install:true, uninstall: true) {
        section ("Device Selection") {
        	href "deviceDiscoveryPage",  title: hrefTitle, description: "Tap to open" 
        }
...
def setServiceDevice(d) {
	log.debug("service device ${d}")
    //if (state.serviceDevices) {
    //	state.serviceDevices = [];
    
	//state.serviceDevice = d;
}

def getServiceDevices(){
	def d = getChildDevices();
    def ret = [];
    d.each { device-> 
    	log.debug "DeviceId ${device.deviceNetworkId.toString()}"
        def a = device.getHost()
        log.debug "got host >>> ${a}"
    }
}

def getServiceDevice(id) {
	def d = getChildDevices();
    return d[0].getServiceDevice(id);
}
/**
